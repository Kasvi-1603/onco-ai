...
import { uploadFiles, startAnalyze, getPipelineStatus } from "../lib/api";
  ...
  const handleUploadAndAnalyze = async (e: React.FormEvent, useDemo = false) => {
    ...
    try {
      const { session_id } = await uploadFiles(
        useDemo ? [] : files,
        useDemo ? "Demo patient with lung adenocarcinoma and EGFR mutation." : symptoms,
        useDemo
      );
                    ...
                    <div className="space-y-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>
                          Supports Pathology PDFs, NGS results, radiology reports
                        </p>
                      </div>
                    </div>
                  </div>
