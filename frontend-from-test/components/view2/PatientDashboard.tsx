...
export default function PatientDashboard({ sessionId }: PatientDashboardProps) {
  ...
  const [activeTab, setActiveTab] = useState<"summary" | "symptoms" | "meds" | "chat" | "vitals" | "docs" | "education">("summary");
  ...
  if (error) {
    const apiErr = (error as any).body;
    const isLocked = (error as any).status === 403 || apiErr?.error === "not_shared" || apiErr?.detail?.error === "not_shared";
