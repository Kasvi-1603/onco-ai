...
def replace_trials(trials: list[dict[str, Any]]) -> int:
    """Replace trial cache with externally fetched normalized trial records."""
    if not trials:
        return 0
    with get_conn() as conn:
        conn.execute("DELETE FROM trials_cache")
        for row in trials:
            conn.execute(
                "INSERT OR REPLACE INTO trials_cache (nct_id, data_json) VALUES (?, ?)",
                (row["nct_id"], json.dumps(row)),
            )
    return len(trials)
