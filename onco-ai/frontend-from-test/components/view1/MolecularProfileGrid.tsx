...
export default function MolecularProfileGrid({ profile }: MolecularProfileGridProps) {
  const { pathology, genomic, imaging, missing_fields, source_snippets } = profile;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="rounded-2xl p-5 shadow-3xs bg-[#f8fafc] border border-slate-100 border-t-4 border-t-indigo-500/80">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              ...
              {renderField("Margins", pathology.margins ?? pathology.surgical_margin, "margins")}
              {renderField("Tumor Size", (pathology.size_mm ?? pathology.tumor_size_mm) ? `${pathology.size_mm ?? pathology.tumor_size_mm} mm` : null, "size_mm")}
            </div>
          </div>
          ...
          <div className="rounded-2xl p-5 shadow-3xs bg-[#f0faf5] border border-emerald-100 border-t-4 border-t-emerald-500/80 flex flex-col justify-between">
            <div>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {renderField("Primary Lobe", imaging.lobe ?? imaging.tumor_lobe, "lobe")}
                {renderField("Nodal Stage", imaging.n_stage, "n_stage")}
                {renderField("Pleural Invasion", imaging.pleural_invasion != null ? (imaging.pleural_invasion ? "Present" : "None Detected") : null, "pleural_invasion")}
                {renderField("Metastases", (() => { const sites = imaging.metastasis_sites; if (!sites || sites.length === 0) return "None Detected"; return sites.join(", "); })(), "metastasis_sites")}
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-12 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-1">
                ...
                {/* PD-L1 Gauge */}
                <Pdl1RadialGauge value={(genomic.pd_l1 ?? genomic.pd_l1_percent) ?? 0} />
                {/* TMB bar */}
