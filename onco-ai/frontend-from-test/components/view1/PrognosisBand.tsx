...
interface PrognosisBandProps {
  stats: PrognosisStats | null | undefined;
}

export default function PrognosisBand({ stats }: PrognosisBandProps) {
  if (!stats) {
    return (
      <div className="rounded-lg shadow-md flex items-center justify-center p-8" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
        <p className="text-sm italic" style={{ color: "var(--text-muted)" }}>Prognosis data not yet available.</p>
      </div>
    );
  }
